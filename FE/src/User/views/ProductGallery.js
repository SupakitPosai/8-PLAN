...
export default class ProductGallery extends Component {
        ...
        .then(
          (result) => {
            ...
            for (let index = 0; index < 15; index++) {
              ...
              if (index === 1) {
                this.setState({ product: ee });
              }
            }
            var ii = Math.ceil(result.length / 15);
            // var ii = 3;
            var dd = [];
            var iti1 = 0;
            var iti2 = 14;
            for (let ii1 = 1; ii1 <= ii; ii1++) {
              dd.push({
                id: ii1,
                to: iti1,
                fo: iti2,
  ...
  npage(nn, nn2) {
    var ppp = this.state.productall;
    var ee = [];
    for (let index = nn; index <= nn2; index++) {
      if (ppp[index] != undefined) {
        ee.push(ppp[index]);
      }
    ...
    return (
      <div>
          ...
          <Carousel>
            {this.state.product1.map((product, index) => (
              <Carousel.Item key={index}>
                <Link to={"/DetailProduct/" + product.id_product}>
              ...
              <Col>
                <div style={{ marginRight: "1rem" }}>
                  <center>
                    ...
                    <h1
                      className="namegaller2m"
                      style={{ color: "#FF9500", fontWeight: "bold" }}
                    >
                      {this.state.gallery[0].name_gallery}
                    </h1>
                  </center>
                </div>
              ...
              <Col>
                <Row>
                  {this.state.product.map((product, index) => (
                    <div
                      key={index}
                      className={this.state.productCol}
                      // style={{ width: "calc(100% / 2 + 0px)" }}
                    >
                      <Link
                        className="Ca"
                        to={"/DetailProduct/" + product.id_product}
                      >
                        <Card className="CC" style={{ border: "unset" }}>
                          ...
                          <div style={{ marginTop: "1rem" }}>
                            <h3 className="nameCut npm11">
                              {product.name_product_th}
                            </h3>
            ...
            {this.state.page.length > 1 && (
              <Row>
                <Col style={{ textAlign: "center" }}>
                  {this.state.page.map((pa) => (
                    <a
                      href="##"
                      onClick={() => {
                        this.npage(pa.to, pa.fo);
                      }}
                    >
                      {pa.id + " "}
                    </a>
        ...
        <Modal show={this.state.show} centered>
          <Modal.Header>
            <Modal.Title>เลือกประเภท</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {this.state.galleryDiv2}
